...
class SuchmaschineDAO extends DAO {
    ...
    public function fetchSuchmaschineByID($id) {
        ...
        $suchmaschineQuery = $mysqli->prepare("SELECT sm.id, sm.name, sm.url, sm.positionenJeSeite
                                               FROM suchmaschine sm
                                               WHERE sm.id = ?
                                               LIMIT 1");
        $suchmaschineQuery->bind_param("i", $id);
        $suchmaschineQuery->execute();
        $suchmaschineID    = 0;
        $name              = "";
        $url               = "";
        $positionenJeSeite = 0;
        $suchmaschineQuery->bind_result($suchmaschineID, $name, $url, $positionenJeSeite);
        $suchmaschineQuery->fetch();
        $suchmaschineQuery->close();
        $parameter = $this->fetchParameterBySuchmaschinenID($id);
        return new Suchmaschine($suchmaschineID, $name, $url, $positionenJeSeite, $parameter);
    }
